...
document.addEventListener('DOMContentLoaded', () => {
 ...
 //test 2 putting and changing numbers in box
  textBoxChanged()
});


let textBox = document.querySelector('#text-input')
// this gets an array of all the cells 
...
//4th Q setting numbers in the grid then lay over to written form 
let gridChanged = () => {
  let textString = ''
  cells.forEach((cell) => {
...
let canPlace = (board, row, col, value) => {
	...
	// checking the colum returns false so not okay to place 
	let i
	for(i = 0; i < 9; i++){
  	...
  	// checking the row returns false so not okay to place 
	let j
	for(j=0; j < 9; j++){
...
// returns board if true or false if cant finish
...
let generateBoard = (values) => {
    ...
    //when i get's to 9 moves on to next boardrow
     if(i % 9 === 0) {
       boardRow += 1
     }
...
//creating array string of solved numbers 
   let i
	let j
	let solutionString = ''
  ...
  // placing solved numbers inthere place
